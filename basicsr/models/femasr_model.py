...
class FeMaSRModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('sdc_opt'):
            self.cri_sdc = build_loss(train_opt['sdc_opt']).to(self.device)
            self.model_to_device(self.cri_sdc)
        else:
            self.cri_sdc = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.LQ_stage:
            ...
            self.output, l_codebook, l_semantic, _ = self.net_g(self.lq) 
        ...
        if self.cri_trgtv:
            l_tv = self.cri_trgtv(self.output)
            l_g_total += l_tv
        ...
        if self.cri_trgdc:
            l_dc = self.cri_trgdc(self.output)
            l_g_total += l_dc
            loss_dict['l_trgdc'] = l_dc

        if self.cri_sdc:
            l_sdc = self.cri_sdc(self.output, self.gt)
            l_g_total += l_sdc
    ...
    def test(self):
        ...
        # print(lq_input.shape)
        if h*w < min_size:
            output = net_g.test(lq_input)
            self.output = output[:, :, :h, :w]
        else:
    ...
    def test_tsne(self):
        ...
        _, _, h, w = lq_input.shape
        if h*w < min_size:
            self.tsne = net_g.test_tsne(lq_input)
            print(self.tsne.shape)
        else:
            self.output = net_g.test_tile(lq_input)
        self.net_g.train()
    ...
    def tsne_tensor(self, dataloader, current_iter, tb_logger, save_img):
        dataset_name = dataloader.dataset.opt['name']
        with_metrics = self.opt['val'].get('metrics') is not None
        if with_metrics:
            self.metric_results = {metric: 0 for metric in self.opt['val']['metrics'].keys()}
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if self.opt['val'].get('grids', False):
                self.grids_inverse()
            if idx == 0:
                current_tensor = self.tsne
            else:
                current_tensor = torch.cat([current_tensor, self.tsne], dim=0)
            del self.tsne
            if idx == 499:
                break
            # torch.cuda.empty_cache()
        save_img_path = osp.join(self.opt['path']['visualization'],
                                                 f'{dataset_name}.pth')
        torch.save(current_tensor, save_img_path)   
